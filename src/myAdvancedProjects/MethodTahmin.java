 ...
 */
    ...
    private static void methodGetir(List<String> methodlar, int index) {
        ...
        do{
            ...
            if (!tahminEdilecekMethod.toString().contains(str)) {//tahminEdilecekFilm'de kullanıcıdan alınan harf yoksa tahmin sayısı artılıdı
                tahminSayısı++;
            }
            for (int i = 0; i <tahminEdilecekMethod.length() ; i++) {
                if(methodlar.get(index).charAt(i)==harf){ //tahmin listeki edilen harf filmde varsa
            ...
            System.out.println(tahminEdilecekMethod);
        }while ((tahminSayısı-dogruTahminSayısı) != 2*methodlar.get(index).length());//yanlış sayısı verilen hakka ulaşana kadar do body calışır
